...
// File: burger.cpp
// Purpose: This file contains the member functions for the burger
//          class.
...
ostream& operator<<(ostream & os, const burger & b)
{
  ...
  os << b.m_name << ": " << "(" << b.m_burger << ", " << b.m_bacon << ", " 
     << b.m_pickle << ", " << b.m_hasCheese << ", " << b.m_hasSauce << "," 
     << b.m_hasPathogen << ") " << "$" << b.m_price;

  return os;
}

// constructor to allow user to initialize burger values
burger::burger(const short burger, const short baconOz, const short pickle, 
      const bool containsCheese, const bool containsSauce, const bool
      containsPathogen)
{
  // allow values to be initialized
  m_burger = burger;
  m_bacon = baconOz;
  m_pickle = pickle;
  m_hasCheese = containsCheese;
  m_hasSauce = containsSauce;
  m_hasPathogen = containsPathogen;

  // initialize a burger name based upon input to constructor
  m_name = generateBurgerName();

  // give price to burger based upon input to constructor
